...
<body class="bg-zinc-950 text-gray-100 antialiased overflow-x-hidden">
    ...
    @php
        ...
        $navLinks = [
            ['path' => '/', 'icon' => 'fa-gauge-high', 'label' => 'DASHBOARD', 'match' => fn($p) => $p === '/', 'roles' => ['superadmin', 'admin', 'kasir']],
            ['path' => '/kasir', 'icon' => 'fa-cash-register', 'label' => 'KASIR', 'match' => fn($p) => str_starts_with($p, 'kasir'), 'roles' => ['superadmin', 'admin', 'kasir']],
            ['path' => '/items', 'icon' => 'fa-boxes-stacked', 'label' => 'STOK', 'match' => fn($p) => str_starts_with($p, 'items'), 'roles' => ['superadmin', 'admin']],
            ['path' => '/admin/bookings', 'icon' => 'fa-calendar-check', 'label' => 'RESERVASI', 'match' => fn($p) => str_starts_with($p, 'admin/bookings'), 'roles' => ['superadmin', 'admin'], 'badge' => $pendingBookingsCount, 'badge_color' => 'orange'],
            ['path' => '/admin/feedbacks', 'icon' => 'fa-comment-dots', 'label' => 'FEEDBACK', 'match' => fn($p) => str_starts_with($p, 'admin/feedbacks'), 'roles' => ['superadmin', 'admin'], 'badge' => $unreadFeedbackCount, 'badge_color' => 'purple'],
            ['path' => '/expenses', 'icon' => 'fa-money-bill-transfer', 'label' => 'PENGELUARAN', 'match' => fn($p) => str_starts_with($p, 'expenses'), 'roles' => ['superadmin', 'admin']],
        ];
        $moreLinks = [
            ['path' => '/reports', 'icon' => 'fa-chart-line', 'label' => 'Laporan Keuangan', 'match' => fn($p) => str_starts_with($p, 'reports'), 'roles' => ['superadmin', 'admin']],
            ['path' => '/customers', 'icon' => 'fa-users', 'label' => 'Data Pelanggan', 'match' => fn($p) => str_starts_with($p, 'customers'), 'roles' => ['superadmin', 'admin']],
            ['path' => '/riwayat-struk', 'icon' => 'fa-file-invoice', 'label' => 'Riwayat & Struk', 'match' => fn($p) => str_starts_with($p, 'riwayat-struk'), 'roles' => ['superadmin', 'admin']],
            ['path' => '/users', 'icon' => 'fa-user-shield', 'label' => 'Manajemen User', 'match' => fn($p) => str_starts_with($p, 'users'), 'roles' => ['superadmin']],
            ['path' => '/settings', 'icon' => 'fa-sliders', 'label' => 'Pengaturan', 'match' => fn($p) => str_starts_with($p, 'settings'), 'roles' => ['superadmin']],
        ];
    @endphp
    ...
    @php
        ...
        foreach ($moreLinks as $ml) {
            if ($ml['match']($currentPath)) {
                $openMoreDefault = true;
                break;
            }
        }
    @endphp
        ...
        <aside class="w-[260px] flex-shrink-0 bg-zinc-900/60 backdrop-blur-md border-r border-white/8 hidden lg:flex flex-col">
            ...
            <div class="px-6 py-7 border-b border-white/5">
                <a href="/" class="flex items-center gap-3">
                    <div class="w-10 h-10 overflow-hidden rounded-xl shadow-lg shadow-orange-900/40 flex-shrink-0 bg-zinc-800">
            ...
            <nav class="flex-1 px-4 py-5 space-y-1 overflow-y-auto" x-data="{ openMore: {{ $openMoreDefault ? 'true' : 'false' }} }">
                @foreach($navLinks as $nav)
                    @php
                        $isActive = $nav['match']($currentPath);
                        $canSee = in_array(auth()->user()->role ?? '', $nav['roles'] ?? ['superadmin', 'admin', 'kasir']);
                    @endphp
                    @if($canSee)
                        <a href="{{ $nav['path'] }}"
                ...
                <div>
                    ...
                    <div x-show="openMore" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="mt-1 ml-3 pl-3 border-l border-white/8 space-y-0.5">
                        @foreach($moreLinks as $menu)
                            @php
                                $isActive = $menu['match']($currentPath);
                                $canSeeMore = in_array(auth()->user()->role ?? '', $menu['roles'] ?? ['superadmin', 'admin', 'kasir']);
                            @endphp
                            @if($canSeeMore)
                                <a href="{{ $menu['path'] }}"
            ...
            <div class="px-4 py-4 border-t border-white/5">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-orange-500/30 to-orange-700/20 border border-orange-500/20 flex items-center justify-center text-xs font-black text-orange-400 flex-shrink-0">
                        {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 2)) }}
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs font-black text-zinc-200 truncate">{{ auth()->user()->name ?? 'User' }}</p>
                        <p class="text-[9px] uppercase tracking-wider font-black
                            {{ auth()->user()->role === 'superadmin' ? 'text-orange-400' : (auth()->user()->role === 'admin' ? 'text-orange-400' : 'text-blue-400') }}">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-orange-400 hover:bg-orange-900/10 transition-all">
                        <i class="fas fa-right-from-bracket text-xs"></i> Keluar
                    </button>
                </form>
                <p class="text-[8px] uppercase tracking-[0.3em] text-zinc-700 font-black mt-2 text-center">&copy; {{ date('Y') }} Auto Engine</p>
            </div>
        </aside>
        ...
        <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <header class="h-14 flex-shrink-0 flex items-center justify-between px-6 border-b border-white/5 bg-zinc-900/40 backdrop-blur-md">
                <div class="flex items-center gap-3">
                    {{-- Mobile logo --}}
                    <div class="lg:hidden w-8 h-8 overflow-hidden rounded-lg bg-zinc-800">
                        <img src="{{ asset('logo.jpg') }}" alt="Logo" class="w-full h-full object-cover">
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-3">
                    {{-- User role badge (topbar) --}}
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border
                        {{ auth()->user()->role === 'superadmin' ? 'bg-orange-900/20 text-orange-400 border-orange-500/20' : (auth()->user()->role === 'admin' ? 'bg-orange-900/20 text-orange-400 border-orange-500/20' : 'bg-blue-900/20 text-blue-400 border-blue-500/20') }}">
                        <i class="fas {{ auth()->user()->role === 'superadmin' ? 'fa-crown' : (auth()->user()->role === 'admin' ? 'fa-shield' : 'fa-cash-register') }} fa-xs"></i>
                        {{ ucfirst(auth()->user()->role ?? '') }}
                    </span>
                    {{-- Time --}}
